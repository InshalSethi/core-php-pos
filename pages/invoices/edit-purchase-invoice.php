...
$page_title='Inventory | Edit Purchase Invoice';
...
if ( isset($_POST['add_invoice']) ) {
  ...
  $db->update('tbl_purchase_invoice',$invoice_array);


  UpdatePurchaseInvoice($in_id,$grand_total,$invoice_date,$db);
  ...
  if (isset($_POST['old_product_id'])) {
    ...
    $old_ws_sell_rate=$_POST['old_product_sell_rate'];
    $old_retail_rate=$_POST['old_product_retail_rate'];
    ...
    for ($i=0; $i <$old_pro_count ; $i++) { 
      ...
      $pre_pro_arr=array(
                          "sell_price"=>$old_ws_sell_rate[$i],
                          "supplier_price"=>$old_supplier_rate[$i],
                          "new_add_qty"=>$old_new_product_quantity[$i],
                          "retail_price"=>$old_retail_rate[$i],
                          "pro_total"=>$old_product_total[$i]
                        );
    
      $db->where("pur_pro_id",$old_pur_pro_id[$i]);
      $db->update("tbl_purchase_invoice_pro",$pre_pro_arr);
      ...
      $db->where("pro_id",$old_product_id[$i]);
      $db->update("tbl_products",$col_update);
  ...
  if (isset($_POST['pur_inv_product_del_id'])) {
    ...
    for ($i=0; $i <$count_del ; $i++) { 
      ...
      ReutrnProductInStock($pur_del_id[$i],$db);

      // remove from purchase products
      $db->where("pur_pro_id",$pur_del_id[$i]);
...
<html lang="en">
  ...
  <div class="content-wrapper" style="padding: 0px 0px;">
    ...
    <?php 
    $db->where("pur_in_id",$in_id);
    $purchase_data=$db->getOne("tbl_purchase_invoice");
    ?>
    ...
    <div class="row">
      <div class="col-md-6 inv-col">
        <div class="form-group row">
          ...
          <div class="col-sm-9">
            <input type="text" name="invocie_num" value="<?php echo $purchase_data['invoice_num']; ?>" class="form-control form-control-sm" autocomplete="off">
          </div>
        </div>
      </div>
      <div class="col-md-6 inv-col">
    ...
    <div class="table-responsive" >
      <table class="table table-bordered table-hover" id="normalinvoice">
        ...
        <tbody id="addinvoiceItem">
          ...
          $db->where("pur_in_id",$in_id); 
          $in_item=$db->get("tbl_purchase_invoice_pro");
          ...
          <tr class="invoice-row<?php echo $item['pro_id']; ?>">
            <td style="width: 500px">
              ...
              <input type="hidden" class="autocomplete_hidden_value" value="<?php echo $item['pur_pro_id']; ?>" name="old_pur_pro_id[]" >
            </td>
            <td style="width: 80px;">
            ...
            <td style="width: 100px;">
              <input name="old_product_sell_rate[]"  id="item_price_<?php echo $item['pro_id']; ?>" class="item_price_1 price_item form-control form-control-sm"  value="<?php echo $item['sell_price']; ?>" type="text">
            </td>
            <td style="width: 100px;">
              <input name="old_product_retail_rate[]" id="supplier_rate<?php echo $item['pro_id']; ?>" class="form-control form-control-sm" value="<?php echo $item['retail_price']; ?>" onkeyup="NewQuntity('<?php echo $item['pro_id']; ?>');" autocomplete="off"  placeholder="0.00" type="text">
            </td>
            <td style="width: 150px">
              <input class="form-control form-control-sm" name="old_new_product_quantity[]" id="new_product_quantity<?php echo $item['pro_id']; ?>" onkeyup="NewQuntity('<?php echo $item['pro_id']; ?>');" value="<?php echo $item['new_add_qty']; ?>" type="text">

              <input type="hidden" class="autocomplete_hidden_value" value="<?php echo $item['new_add_qty']; ?>" name="pre_product_qty[]" >
            ...
            <td>
              <button class="btn btn-danger btn-rounded btn-icon btn-del" type="button" onclick="deletePreviousRow('<?php echo $item['pro_id']; ?>','<?php echo $item['pur_pro_id']; ?>','<?php echo $item['new_add_qty']; ?>')" value="Delete" ><i class="mdi mdi-delete"></i>
              </button>
            </td>
          </tr>
    ...
    <tfoot>
      ...
      <tr>
        ...
        <td class="text-right" colspan="2">
            <input onkeyup="DiscountCal();" id="total_discount_ammount" class="form-control form-control-sm" name="total_discount" tabindex="-1" value="<?php echo $purchase_data['total_discount']; ?>" autocomplete="off"  type="text">
        </td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td class="text-right" colspan="2">
            <input id="paidAmount" autocomplete="off" class="form-control form-control-sm" name="paid_amount" onkeyup="invoice_paidamount();" value="<?php echo $purchase_data['paid_amount']; ?>" placeholder="0.00" tabindex="7" type="text" readonly>
        </td>
      </tr>
      <tr>                               
          <td style="text-align:right;" colspan="3"><b>Total Due:</b></td>
          <td class="text-right" colspan="2">
              <input id="dueAmmount" class="form-control form-control-sm" name="due_amount" value="<?php echo $purchase_data['total_due']; ?>" readonly="" type="text">
          </td>
      </tr>
    </tfoot>
  </table>                            
